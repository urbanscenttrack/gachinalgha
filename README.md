...
| 효과 | 위치 | 구현 |
|---|---|---|
| **바람(비행 궤적)** | 히어로 · 신념 · 문의 섹션 | 스크롤 속도에 따라 점선 궤적 5줄이 옆으로 흐르고, 멈추면 서서히 제자리로. 로고의 점선 비행 궤적 모티프 |
| 히어로 패럴랙스 | 히어로 배경 사진 | 스크롤 시 사진이 느리게 따라옴 (최대 40px, 1.10배) |
...
바람 효과는 `transform` 과 `opacity` 만 사용하고 requestAnimationFrame 이 스크롤이
멈추면 자동으로 꺼져서, 유휴 상태에서는 CPU 를 쓰지 않습니다.
세기를 조절하려면 `tools/build.py` 의 `_STREAKS` 목록(위치·너비·속도계수)과
site_js 의 감쇠 계수 `0.94` 를 만지면 됩니다.
